...
namespace SGO3
{
    ...
    {
        private string select_all_string = "SELECT * FROM subject";
        private string add_string = "INSERT INTO subject(name) VALUES (@name)";
        private string update_string = "UPDATE subject SET name = @newname WHERE id = @id AND name = @oldname";
        private string delete_string = "DELETE FROM subject WHERE id = @id AND name = @name";
        private NpgsqlConnection conn;
        public SubjectRepository(NpgsqlConnection _conn)
        {
            conn = _conn;
        }
